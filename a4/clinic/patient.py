...
class Patient:
    def __init__(self, phn: int, name: str, DoB: str, phone: str, email: str, address: str, autosave: bool = False) -> None:
    ...
    # Return True if all attributes of self and other are equal.
        ...
        return self.phn == other.phn and self.name == other.name and self.DoB == other.DoB \
    ...
    # Returns a formatted string listing a Patient's attributes
        ...
        return f'PHN: {self.phn}, Name: {self.name}, DoB: {self.DoB}, Phone: {self.phone}, Email: {self.email}, Address: {self.address}'

    def __repr__(self) -> str:
    # Returns a formatted string with Patient attributes

        return f'Patient({repr(self.phn)}, {repr(self.name)}, {repr(self.DoB)}, {repr(self.phone)}, {repr(self.email)}, {repr(self.address)}'
